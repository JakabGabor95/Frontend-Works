...
headOfSnake = snake[snake.length-1];
if(headOfSnake.widthP === apple[0] && headOfSnake.heightP === apple[1]) {
    const newBody  = {
        ...
        snake.push(newBody);
        apple[0]=Math.floor(Math.random() * ((canvasWidth-10)/10))*10; 
        apple[1]=Math.floor(Math.random() * ((canvasHeight-10)/10))*10; 
        appleCount++;
...
function hasCollision() {
        ...
        let headOfSnake=snake[snake.length-1];
        if(headOfSnake.widthP===canvasWidth || headOfSnake.widthP<0 || headOfSnake.heightP===canvasHeight || headOfSnake.heightP<0){
